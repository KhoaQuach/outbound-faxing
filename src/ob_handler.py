...
class OB_Handler(Thread):
    ...
    def __init__(self, outbound_fax_id, destination_number, source_number, \
        ...
        self._socket = FS_Socket(ob_global_vars.FS_HOST, ob_global_vars.FS_SOCKET_PORT, ob_global_vars.FS_SOCKET_PASSWORD)
    ...
    def send_out_fax(self):
        ...
        """
        ob_logger.debug("Connected: " + str(self._socket.connected()))

        if self._socket.connected():
            command_args = build_fs_ob_fax_command_args(ob_global_vars.VOIP_GW, self._destination_number, self._fax_file)
            ob_logger.debug(command_args)
    ...
    def update_ob_fax_status(self, outbound_fax_id, outbound_fax_status_id, num_attempts, result_code, result_message):
        """
            Update the database outbound fax record with appropriate 
            progress and status
        """
        ob_logger.debug(__function__ + " entering")
        ...
        ob_logger.debug(__function__ + " exiting")
    ...
    def wait_for_hangup_events(self):
        ...
        while True: 
            ...
            ob_logger.debug("Number of tries: " + str(self._num_attempts))
            if self._max_attemtps < self._num_attempts:
                ob_logger.warning("Max out number of attempts!")
                update_ob_fax_status(self._outbound_fax_id, 2, self._num_attempts, -1, '')
                return 

            if not self._socket.connected():
                ob_logger.error("Socket session disconnected!")
                update_ob_fax_status(self._outbound_fax_id, 2, self._num_attempts, -1, '')
                return 
            ...
            if e is not None:
                ...
                if event_name == "BACKGROUND_JOB":
                    if e.getHeader("Job-UUID") == self._job_uuid: 
                        ...
                        ob_logger.debug("Result of call is {0}".format(call_result))

                elif event_name == "CHANNEL_HANGUP" or event_name == "CHANNEL_HANGUP_COMPLETE":
                    ob_logger.debug("Call has been hung up")
                    ...
                    ob_logger.debug("fax_result_code: " + self._fax_result_code)
                    ob_logger.debug("fax_result_text: " + self._fax_result_text)
                    ob_logger.debug("fax_success: " + self._fax_success)
                    ob_logger.debug("fax_document_total_pages: " + self._fax_document_total_pages)
                    ob_logger.debug("fax_document_transferred_pages: " + self._fax_document_transferred_pages)
                    ob_logger.debug("fax_image_resolution: " +  self._fax_image_resolution)
                    ob_logger.debug("fax_image_size: " + self._fax_image_size )
                    ob_logger.debug("fax_local_station_id: " +  self._fax_local_station_id )
                    ob_logger.debug("fax_remote_station_id: " + self._fax_remote_station_id)
                    ob_logger.debug("fax_transfer_rate: " + fax_transfer_rate)
                    ob_logger.debug("fax_bad_rows: " + fax_bad_rows)
                    ob_logger.debug("t38_gateway_format: " + t38_gateway_format)
                    ob_logger.debug("t38_peer: " + t38_peer)
                    ob_logger.debug("hangup_cause: " + self._hangup_cause)

                    update_ob_fax_status(self._outbound_fax_id, 1, self._num_attempts, self._fax_result_code, self._fax_result_text)
